(function() {
  ...
  // con panel avanzado de selección de color, degradado y dirección condicional
  ...
  function openColorPanel(block, innerBtn) {
    ...
    // Contenedor direcciones oculto inicialmente
    const dirContainer = document.createElement('div');
    Object.assign(dirContainer.style, { display: 'none', flexDirection: 'row', gap: '6px' });
    ...
    function updateBackground() {
      ...
      const selEmoji = [...dirContainer.children].find(b => b.style.opacity === '1').textContent;
      let css;
      if (selEmoji === '↕️') css = `linear-gradient(${c1}, ${c2})`;
      else if (selEmoji === '↔️') css = `linear-gradient(90deg, ${c1}, ${c2})`;
  ...
  btn.addEventListener('click', () => {
    ...
    document.addEventListener('touchmove', e => {
      ...
      document.dispatchEvent(new MouseEvent('mousemove',{clientX:t.clientX,clientY:t.clientY}));e.preventDefault();
    }, { passive: false });
