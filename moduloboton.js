  ...
  btn.addEventListener('click', () => {
    ...
    handle.addEventListener('click', e => {
      ...
      if (icon === '🎨') {
        ...
        picker.addEventListener('input', ev => innerBtn.style.background = ev.target.value);
    ...
    document.addEventListener('touchmove', e => {
      if (!dragging) return;
      const t = e.touches[0]; onMove({ clientX: t.clientX, clientY: t.clientY }); e.preventDefault();
    }, { passive: false });
