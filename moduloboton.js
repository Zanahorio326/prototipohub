(function() {
  ...
  function openPaintPanel(block, innerBtn) {
    ...
    // Direcciones ocultas
    const dirContainer = document.createElement('div'); dirContainer.style.display='none'; dirContainer.style.gap='6px'; dirContainer.style.display='flex'; dirContainer.style.flexDirection='row';
    ['↕️','↔️','⭕'].forEach(e=>{ const b=document.createElement('button'); b.textContent=e; b.style.opacity='0.5'; b.style.cursor='pointer'; b.addEventListener('click',()=>{dirContainer.querySelectorAll('button').forEach(x=>x.style.opacity='0.5'); b.style.opacity='1'; update();}); dirContainer.appendChild(b);}); dirContainer.children[0].style.opacity='1'; panel.appendChild(dirContainer);
    function update(){ const c1=color1.value; if(!gradientToggle.classList.contains('active')){ innerBtn.style.background=c1; return;} const c2=color2.value; const sel=[...dirContainer.children].find(x=>x.style.opacity==='1').textContent; let css; if(sel==='↕️')css=`linear-gradient(${c1},${c2})`; else if(sel==='↔️')css=`linear-gradient(90deg,${c1},${c2})`; else css=`radial-gradient(circle,${c1},${c2})`; innerBtn.style.background=css;};
    color1.addEventListener('input',update); color2.addEventListener('input',update);
    gradientToggle.addEventListener('click',()=>{ const a=gradientToggle.classList.toggle('active'); color2.style.display=a?'block':'none'; dirContainer.style.display=a?'flex':'none'; update(); });
    document.body.appendChild(panel);
    document.addEventListener('mousedown',function o(e){ if(!panel.contains(e.target)){ panel.remove(); document.removeEventListener('mousedown',o);} });
  }

  function openBorderPanel(block){
    const panel=document.createElement('div');
    ...
    // Color contorno
    const borderColor=document.createElement('input'); borderColor.type='color'; borderColor.value='#0056b3'; panel.appendChild(borderColor);
    // Grosor contorno
    const thickness=document.createElement('input'); thickness.type='range'; thickness.min='1'; thickness.max='10'; thickness.value='1'; panel.appendChild(thickness);
    function updateBorder(){ block.style.border=`${thickness.value}px solid ${borderColor.value}`;} borderColor.addEventListener('input',updateBorder); thickness.addEventListener('input',updateBorder);
    document.body.appendChild(panel);
    document.addEventListener('mousedown',function o(e){ if(!panel.contains(e.target)){ panel.remove(); document.removeEventListener('mousedown',o);} });
  }
  ...
  btn.addEventListener('click',()=>{
    ...
    const handle=document.createElement('div'); handle.classList.add('block-handle'); const handleSize=24; Object.assign(handle.style,{position:'absolute',width:handleSize+'px',height:handleSize+'px',bottom:-(handleSize/2)+'px',right:-(handleSize/2)+'px',borderRadius:'50%',background:'#fff',border:'1px solid #0056b3',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',userSelect:'none',fontSize:'14px',boxSizing:'border-box'}); handle.textContent=getCurrentIcon(); block.appendChild(handle);
    let dragging=false,startX,startY,origX,origY,origW,origH;
    handle.addEventListener('mousedown',e=>{ const icon=getCurrentIcon(); if(icon==='🎨'){ openPaintPanel(block,innerBtn);return;} if(icon==='🖌️'){ openBorderPanel(block); return;} if(icon==='💠'||icon==='↘️'){ dragging=true; startX=e.clientX; startY=e.clientY; origX=block.offsetLeft; origY=block.offsetTop; origW=block.offsetWidth; origH=block.offsetHeight; handle.style.cursor='grabbing'; e.stopPropagation(); e.preventDefault(); }});
    document.addEventListener('mousemove',e=>{ if(!dragging)return; const dx=e.clientX-startX, dy=e.clientY-startY; const icon=getCurrentIcon(); if(icon==='💠'){ block.style.left=origX+dx+'px'; block.style.top=origY+dy+'px'; } else if(icon==='↘️'){ const newW=Math.max(origW+dx,margin*2+20), newH=Math.max(origH+dy,margin*2+20); block.style.width=newW+'px'; block.style.height=newH+'px'; innerBtn.style.width=newW-margin*2+'px'; innerBtn.style.height=newH-margin*2+'px'; } });
    document.addEventListener('mouseup',()=>{ if(dragging){ dragging=false; handle.style.cursor='pointer'; } });
    handle.addEventListener('touchstart',e=>{ const t=e.touches[0],icon=getCurrentIcon(); if(icon==='🎨'){ openPaintPanel(block,innerBtn);return;} if(icon==='🖌️'){ openBorderPanel(block);return;} if(icon==='💠'||icon==='↘️'){ dragging=true; startX=t.clientX; startY=t.clientY; origX=block.offsetLeft; origY=block.offsetTop; origW=block.offsetWidth; origH=block.offsetHeight; } e.preventDefault();},{passive:false});
